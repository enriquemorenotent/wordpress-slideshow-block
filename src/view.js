const configureSlider = (slideshow) => {
	...
	const handlePreviousClick = () =>
		handleChangeActive((activeSlide - 1 + slides.length) % slides.length);

	const handleNextClick = () =>
		handleChangeActive((activeSlide + 1) % slides.length);
...
const configureSliders = () => {
	const slideshows = document.querySelectorAll(".wp-block-emt-slideshow-block");
